 ...
 * @author santi
 */
public class HeavyRushImproved extends AbstractionLayerAI {
    ...
    // If we have a base: train workers until we have 3 workers (improved from 1)
    // If we have a barracks: train heavy
    // If we have a worker: do this if needed: build base, build barracks, harvest resources

    public HeavyRushImproved(UnitTypeTable a_utt) {
        this(a_utt, new AStarPathFinding());
    }
    
    
    public HeavyRushImproved(UnitTypeTable a_utt, PathFinding a_pf) {
        super(a_pf);
        reset(a_utt);
    }
    ...
    public AI clone() {
        return new HeavyRushImproved(utt, pf);
    }
